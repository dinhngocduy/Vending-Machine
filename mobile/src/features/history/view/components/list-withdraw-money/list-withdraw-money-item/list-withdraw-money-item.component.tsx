...
import { ListWithDrawMoneyItemProps } from './list-withdraw-money-item.props';

export const ListWithDrawMoneyItemComponent = (props: ListWithDrawMoneyItemProps) => {
  const { item, onPress } = props;
  const onPressItem = () => {
    onPress(item, TypeTask.WITHDRAW_MONEY);
  };
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={onPressItem}>
        <View style={styles.row}>
          ...
          <Text style={styles.money_text}>{`-${item.money} đ`}</Text>
        </View>
